            ...
            <div class="ut2-lmw {$menus.user_class}">
                ...
                {foreach $menus.menus as $item}
                    <div class="ut2-lfl {$item.class}{if $item.active} ut2-lm-active-item{/if}">
                        ...
                        {if $item.subitems}
                            <i></i>
                            <div class="ut2-slw{if $props.abt_menu_ajax_load == 'Y'} abt__ut2_am" {$unique_id = "{$unique_part}_{$item.param_id}"} id="{$unique_id}_{$smarty.const.DESCR_SL}_{$settings.abt__device}"{else}"{/if}>
