        ...
        {hook name="products:out_of_stock_block"}
            ...
            {if !$details_page}
                {if (!$product.hide_stock_info && !(($product_amount <= 0 || $product_amount < $product.min_qty) && ($product.avail_since > $smarty.const.TIME)))}
                    <span class="ty-qty-out-of-stock ty-control-group__item" id="out_of_stock_info_{$obj_prefix}{$obj_id}"><i class="ut2-icon-outline_off-circle"></i>{$out_of_stock_text}</span>
                {/if}
            ...
            {hook name="products:prices_block"}
                ...
                {elseif $product.zero_price_action == "A" && $show_add_to_cart}
                    ...
                    <div class="ty-price-curency-input">
                        {if $base_currency.after != "YesNo::YES"|enum}{$base_currency.symbol nofilter}{/if}
                        <input class="ty-price-curency__input" type="text" size="3" name="product_data[{$product.product_id}][price]" value="" />
                        {if $base_currency.after == "YesNo::YES"|enum}{$base_currency.symbol nofilter}{/if}
                    </div>
                    </span>
                    ...
                    {if ($product_amount > 0 && $product_amount >= $product.min_qty) && $settings.General.inventory_tracking !== "YesNo::NO"|enum || $details_page}
                        {if (
                            ...
                            <div class="product-list-field ty-mb-m">
                                ...
                                <span id="qty_in_stock_{$obj_prefix}{$obj_id}" class="ty-qty-in-stock ty-control-group__item">
                                    ...
                                    {else}
                                    	<span class="on_backorder"><i class="ut2-icon-notifications_none-24px"></i>{__("on_backorder")}</span>
                                    {/if}
                                </span>
                            </div>
                        {elseif $settings.General.inventory_tracking !== "YesNo::NO"|enum && $settings.General.allow_negative_amount != "YesNo::YES"|enum}
                            <div class="ty-control-group product-list-field">
                                ...
                                {/if}
                                <span class="ty-qty-out-of-stock ty-control-group__item"><i class="ut2-icon-outline_off-circle"></i>{$out_of_stock_text}</span>
                            </div>
                        {/if}
                    {/if}
                {/if}
