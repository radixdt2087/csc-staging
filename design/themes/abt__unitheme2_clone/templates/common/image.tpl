...
{if $image_data.image_path}
    ...
    {hook name="products:product_image_object"}
        <img class="ty-pict {$valign} {$class}{if $generate_image} ty-spinner{/if} cm-image" {if $obj_id && !$no_ids}id="det_img_{$obj_id}"{/if} {if $generate_image}data-ca-image-path="{$image_data.image_path}"{/if} {if $lazy_load}src="{$smarty.const.ABT__UT2_LAZY_IMAGE}" data-{/if}src="{if $generate_image}{$images_dir}/icons/spacer.gif{else}{$image_data.image_path}{/if}" {if $image_onclick}onclick="{$image_onclick}"{/if} {$image_additional_attrs|render_tag_attrs nofilter}/>
