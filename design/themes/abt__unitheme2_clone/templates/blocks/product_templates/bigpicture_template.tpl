                ...
                {if !$no_images}
                    <div class="ut2-pb__img {if $product.image_pairs|@count < 1} ut2-pb__no-thumbs{/if} cm-reload-{$product.product_id} {if $settings.Appearance.thumbnails_gallery == "YesNo::YES"|enum}ut2-pb__as-gallery{else}ut2-pb__as-thumbs{/if}" data-ca-previewer="true" id="product_images_{$product.product_id}_update">
                        {if $settings.abt__device == "mobile"}
                            {include file="views/products/components/product_images.tpl" product=$product show_detailed_link="YesNo::YES"|enum image_width=$th_size image_height=$th_size thumbnails_size=50}
