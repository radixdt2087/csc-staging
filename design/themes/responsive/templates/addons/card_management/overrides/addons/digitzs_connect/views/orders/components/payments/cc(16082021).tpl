...
<div id="savedDefaultCards">
    <div class="accordiondefault">
        <div class="card_name"> <input type="radio" name="payment_info[card_token]" value="{$defaultCard->data->id}"> <div class="ty-cc-icons__item cm-cc-{$defaultCard->data->attributes->card->type} active"><span class="ty-cc-icons__icon {if $defaultCard->data->attributes->card->type == 'amex'} american-express {else} {$defaultCard->data->attributes->card->type} {/if}"> &nbsp; </span></div> {ucfirst($defaultCard->data->attributes->card->type)} card ending with {$defaultCard->data->attributes->card->number|substr:-4}</div>
