...
<div id="savedDefaultCards">
    <div class="accordiondefault">
        <div class="row card_name">
            <div class="span12">
                <div class="row">
                    <div class="span1" style="margin-left: 50px;">
                        <input type="hidden" name="payment_info[card_token]" value="{$defaultCard->data->id}">                    <div class="ty-cc-icons__item cm-cc-{$defaultCard->data->attributes->card->type} active" style="      margin-top: -15px; margin-left: -5px;"><span class="ty-cc-icons__icon {if $defaultCard->data->attributes->card->type == 'amex'} american-express {else} {$defaultCard->data->attributes->card->type} {/if}"> &nbsp; </span></div>
                    </div>
                    <div class="span20" style="margin-left: 110px;">
                        <b>{ucfirst($defaultCard->data->attributes->card->type)}</b> card ending with <b> {$defaultCard->data->attributes->card->number|substr:-4}</b></p>
                    </div>
                </div>
                <div class="row">
                    <div class="span15" style="margin-left:50px;"> 
...
<div id="savedPreviousCards" style="display:none;">
    ...
    {foreach from=$card_details key=key item=item}
        <div id="cardDetails">
            <div class="accordion">
                <div class="row card_name" id="{$item->data->id}">
                    <div class="span8">
                        <div class="row">
                            <div class="span1" style="margin-left: 50px;" id="{$item->data->id}">
                                <input type="radio" name="payment_info[card_token]" value="{$item->data->id}"><div class="ty-cc-icons__item cm-cc-{$item->data->attributes->card->type} active" style="margin-top: -15px;"><span class="ty-cc-icons__icon {if $item->data->attributes->card->type == 'amex'} american-express {else} {$item->data->attributes->card->type} {/if}"> &nbsp; </span></div>
                            </div>
                            <div class="span20" style="margin-left: 130px;margin-top: -10px;">
                                <b>{ucfirst($item->data->attributes->card->type)}</b> card ending with <b> {$item->data->attributes->card->number|substr:-4}</b></p>
                            </div>
                        </div>
                    </div>
                    <div class="span4">
                        <div class="row">
                            <div style="text-align: center;margin-top: -4px;" id="expiry{$item->data->id}">{ucfirst($item->data->attributes->card->holder)}</div>
                        </div>                
                    </div>
                    <div class="span4">
                        <div class="row">
                            <div style="text-align: end;margin-top: -4px;" id="expiry{$item->data->id}">{$item->data->attributes->card->expiry|substr:0:2}/{$item->data->attributes->card->expiry|substr:2}<i class="ut2-icon-outline-expand_more"></i></div>
                        </div>                
                    </div>
                    <!-- <div class="span2 card_action"><a class="button" onclick="savedCards()">Change</a></div> -->
                </div>
            </div>
            <!-- <div class="accordion">
                <div class="card_name" id="{$item->data->id}"> <input type="radio" name="payment_info[card_token]" value="{$item->data->id}"> <div class="ty-cc-icons__item cm-cc-{$item->data->attributes->card->type} active"><span class="ty-cc-icons__icon {if $item->data->attributes->card->type == 'amex'} american-express {else} {$item->data->attributes->card->type} {/if}"> &nbsp; </span></div> {ucfirst($item->data->attributes->card->type)} card ending with {$item->data->attributes->card->number|substr:-4}</div>
    ...
    <div class="edit_card" id="edit_card" style="display: none;">
        ...
        <div class="panel show">
            <div class="edit_card1 position-data" id="edit_card1" >
                <div class="row">
                    <div class="span6 align-initial"><b>Card Number</b></div>
                    <div class="span6 align-end"><b>Billing Address</b></div>
                </div>
                <div class="row">
                    <div class="span6 align-initial" style="vertical-align: baseline;">
                        <input size="35" type="text" id="credit_card_number_{$id_suffix}" name="payment_infoe[card_number]" value="" class="ty-credit-card__input cm-autocomplete-off ty-inputmask-bdi" />
                        ...
                        <label for="credit_card_number_{$id_suffix}" class="ty-control-group__title cm-cc-number cc-number_{$id_suffix} ">Name on Card</label>
                        <input type="text" id="credit_card_name" name="payment_infoe[cardholder_name]" value="" class="cm-cc-name ty-credit-card__input ty-uppercase" /> <br>
...
<div id="adding_card">
    <div class="accordion">
        <div class="card_name">+ <a >Add a credit card > </a> Wesave accepts all major credit cards.</div>
    </div>
    ...
    <div class="litecheckout__item">
        <div class="clearfix">
            <div class="ty-credit-card cm-cc_form_{$id_suffix}">
                ...
                <!-- <a class="lineheight" id="save_my_card">Save my Card</a> <a class="lineheight" id="unsave_my_card">Dont save my Card</a> -->
...
<script type="text/javascript">
    ...
    $(".button_cart_818").click(function() {
        //alert($('#edit_card1').find("input[name='payment_infoe[card_number]']").val());
