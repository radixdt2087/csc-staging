...
{capture name="main_icon"}
    {if $affiliate_merchant}
        {if $auth.user_id > 0}
            <a class="sd-product-list-link affiliate-product" href="{"products.view&product_id=`$product.product_id`&id=`$product.affiliate_product`"|fn_url}" >
        {else}
             <a href = "javascript:void(0)" class="product-affiliate-redirect">
        {/if}
    {else}
