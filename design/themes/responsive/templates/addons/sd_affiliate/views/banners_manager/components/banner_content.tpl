...
{capture name="banner_content"}
    {if $banner_type == "iframe_content" || $banner_type == "js_content"}
        {if $banner_data.type == "T"}
            ...
            <tr>
                <td style="padding: 5px 10px 10px 10px;" class="valign-top" id="id_cart_{$banner_data.banner_id}_body">
                    ...
                    <div id="id_cart_{$banner_data.banner_id}_link" class="affilate-banner-link"></div>
                </td>
            </tr>
            </table>
        {elseif $banner_data.type == "G"}
            {* Graphic banner's template *}
            {if $banner_data.main_pair.icon.image_type != "application/x-shockwave-flash"}
            <a href="{$banner_data.banner_url}" {if $banner_data.new_window == "Y"}target="_blank"{/if}>
            {/if}
                {include "common/image.tpl"
                    image_id     = $banner_data.image_id
                    images       = $banner_data.main_pair
                    lazy_load    = false
                    no_lazy_load = true
                }
            {if $banner_data.main_pair.icon.image_type != "application/x-shockwave-flash"}
        ...
        {elseif $banner_data.type == "P"}
            ...
            <tr>
                <td align="{$banner_data.align}" style="padding: 5px 10px;">
                    {if $banner_data.banner_url}
                        <a href="{$banner_data.banner_url}" target="{if $banner_data.new_window == "Y" || $banner_type == "iframe_content"}_blank{else}_self{/if}">
                    {/if}
                    {include "common/image.tpl"
                        obj_id       = $banner_data.product_id
                        images       = $banner_data.main_pair
                        image_width  = $image_width|default:$settings.Thumbnails.product_lists_thumbnail_width
            ...
            {/if}
        {/if}
        {if $banner_data.to_price == "Y"}
            {$banner_data.price|format_price:$currencies.$secondary_currency nofilter}
        {/if}
