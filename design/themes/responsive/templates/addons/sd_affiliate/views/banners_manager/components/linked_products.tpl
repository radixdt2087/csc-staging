...
<table class="ty-table margin-top table-width">
<tr>
    <thead>
        <th><input type="checkbox" name="check_all" value="Y" title="{__("check_uncheck_all")}" class="ty-checkbox cm-check-items" /></th>
...
{foreachelse}
    <tr>
        <td colspan="3"><p class="ty-no-items">{__("text_all_products_included", ["[items]" => __("products")])}</p></td>
    </tr>
{/foreach}
</table>
...
{include file="pickers/products/picker.tpl" view_mode="button" but_text=__("add_products_to_section") extra_var="banners_manager.do_add_linked?banner_id=`$banner_id`" display="sd_affiliate" data_id="linked_products_list"}
</div>