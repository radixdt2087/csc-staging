    ...
    {hook name="common:image"}
        {if $show_detailed_link}
            <a id="det_img_link_{$obj_id}"
                {if $image_data.detailed_image_path && $image_id}
                    data-ca-image-id="{$image_id}"
                {/if}
                ...
                {if $image_data.detailed_image_path && !$video}
        ...
        {if $image_data.image_path}
            ...
            {hook name="products:product_image_object"}
                <img {if $obj_id && !$no_ids}id="det_img_{$obj_id}"{/if}
                    ...
                    {if $lazy_load}data-{/if}src="
                        {if $generate_image}
                            {$images_dir}/icons/spacer.gif
                        {else}
                    ...
                    {if $runtime.mode != "view" && ($image_width || $image_height)}
                ...
                {if $show_detailed_link}
                    <svg class="ty-pict__container"
                        aria-hidden="true"
                        width="{$image_data.width}"
                        height="{$image_data.height}"
                        viewBox="0 0 {$image_data.width} {$image_data.height}"
        ...
        {elseif $show_no_image}
            <span class="ty-no-image"
                style="min-width: {$image_width|default:$image_height}px; min-height: {$image_height|default:$image_width}px;">
