{if $products}
	...
	{$th = $t1|default:0 + $t2|default:20 + $t3|default:0 + $t4|default:0 + $t5|default:0 + $t6|default:0 + $t7|default:0 + $t8|default:0 + $pd}
                ...
                {foreach from=$sproducts item="product" name="sproducts"}
                    <div class="ty-column{$columns}"{if $ut2_load_more && $smarty.foreach.sprod.first && $smarty.foreach.sproducts.first} data-ut2-load-more="first-item"{/if}>
                        {if $product}
                            ...
                            <div class="ut2-gl__item {if $settings.abt__ut2.product_list.decolorate_out_of_stock_products == "YesNo::YES"|enum && $product.amount <= 0}out-of-stock{/if}" style="height: {$smarty.capture.abt__ut2_gl_item_height nofilter}px">
                                ...
                                <div class="ut2-gl__body" style="min-height: {if $settings.abt__ut2.product_list.$tmpl.grid_item_height[$settings.abt__device]}{if $settings.abt__ut2.product_list.$tmpl.show_sku[$settings.abt__device] == 'Y'}{$product_height|intval + 16}{else}{$product_height|intval}{/if}{else}{$th}{/if}px">
									...
									{include file="views/products/components/product_icon.tpl" product=$product show_gallery=$settings.abt__ut2.product_list.show_gallery == 'Y' lazy_load=$settings.abt__ut2.general.lazy_load == 'Y'}

                                        {assign var="product_labels" value="product_labels_`$obj_prefix``$obj_id`"}
                                        {$smarty.capture.$product_labels nofilter}

                                        <div class="ut2-gl__buttons" id="{$smarty.capture.abt__service_buttons_id}">
									...
									<div class="ut2-gl__name">
										...
										{if $settings.abt__ut2.product_list.show_rating !="YesNo::YES"|enum}{hook name="products:video_gallery"}{/hook}{/if}
                                    ...
                                    {capture name="product_multicolumns_list_control_data_wrapper"}
                                        {if $show_add_to_cart && $settings.abt__ut2.product_list.$tmpl.show_buttons[$settings.abt__device] == "YesNo::YES"|enum}
                                            <div class="ut2-gl__control {if $settings.abt__ut2.product_list.$tmpl.show_buttons_on_hover[$settings.abt__device] == "YesNo::YES"|enum}hidden{/if} {if !$hide_form && $addons.call_requests.buy_now_with_one_click == "YesNo::YES"|enum && ($auth.user_id || $settings.General.allow_anonymous_shopping == "allow_shopping")}bt-2x{/if}{if $settings.abt__ut2.product_list.$tmpl.show_qty[$settings.abt__device] == "YesNo::YES"|enum && ($settings.General.allow_anonymous_shopping == "allow_shopping" || $auth.user_id) && !$product.has_options} ut2-view-qty{/if}">
