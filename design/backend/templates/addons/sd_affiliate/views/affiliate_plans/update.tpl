...
{if $plan_type == 'AffiliateUserTypes::PARTNER'|enum}
    ...
    <form action="{""|fn_url}" method="post" name="affiliate_plan_form" class="form-horizontal form-edit  {if !$allow_save}cm-hide-inputs{/if}">
        ...
        <input type="hidden" name="affiliate_plan[plan_type]" value="{$plan_type}" />
                ...
                <fieldset>
                    {foreach from=$payout_types key="payout_id" item=payout_data name="payout_types"}
                        ...
                        {if $payout_data.default == "Y"}
                            ...
                            <div class="control-group">
                                <label for="elm_aff_plan_payout_types_{$payout_data.id}" class="control-label">{__($payout_var)}:
                                    {include file="common/tooltip.tpl" tooltip=__($payout_data.tooltip)}
                                </label>
                                <div class="controls">
        ...
        {if $affiliate_plan}
            ...
            <div id="content_multi_tier_affiliates">
                ...
                <table class="table table-middle">
                    <thead>
                        <tr>
                            <th class="center plan-update-check-items">
            ...
            <div id="content_linked_products">
                {if "ULTIMATE"|fn_allowed_for && $allow_save || !"ULTIMATE"|fn_allowed_for}
                    <div class="p-notice">
                        {if "MULTIVENDOR"|fn_allowed_for}
                            {__("affiliate.products_note_mv")}
                        {else}
                            {__("affiliate.products_note")}
                        {/if}
                    </div>
                    <div class="clearfix">
                        <div class="pull-right">
                ...
                <table class="table table-middle">
                    <thead>
                        <tr>
                            <th class="left plan-update-check-items">
                                {include file="common/check_items.tpl" check_target="product"}</th>
                            <th class="plan-update-product-name">{__("product_name")}</th>
                            {if $payout_types}
                                <th class="plan-update-product-commissions">{__("sales_commission")}</th>
                                <th>&nbsp;</th>
                            {/if}
                        </tr>
                    </thead>
                        ...
                        {foreach from=$linked_products key=product_id item=product}
                            <tr>
                                ...
                                {if $payout_types}
                                    ...
                                    <td class="nowrap right plan-update-tools">
                                        {capture name="tools_list"}
            ...
            <div id="content_linked_categories">
                {if "ULTIMATE"|fn_allowed_for && $allow_save || !"ULTIMATE"|fn_allowed_for}
                    ...
                    <div class="clearfix">
                        <div class="p-notice">
                            {if "MULTIVENDOR"|fn_allowed_for}
                                {__("affiliate.categories_note_mv")}
                            {else}
                                {__("affiliate.categories_note")}
                            {/if}
                        </div>
                        <div class="pull-right">
                            ...
                            extra_var="affiliate_plans.add_categories?plan_id=`$id`&selected_section=linked_categories"
                            multiple=true pos=false}
                        </div>
                    </div>
                    {** /Form submit section **}
                {/if}
                ...
                <table class="table table-middle">
                    <thead>
                        <tr>
                            ...
                            <th class="plan-update-category">{__("category")}</th>
                            <th class="plan-update-category-commission">{__("sales_commission")}</th>
                            <th>&nbsp;</th>
                        </tr>
                    </thead>
                    {if $linked_categories}
            ...
            {if fn_allowed_for("MULTIVENDOR")}
                {** Linked vendors **}
                <div id="content_linked_vendors">
                    {if "ULTIMATE"|fn_allowed_for && $allow_save || !"ULTIMATE"|fn_allowed_for}
                        <div class="clearfix">
                            <div class="p-notice">
                                {__("affiliate.vendors_note")}
                            </div>
                            <div class="pull-right">
                                {if $linked_vendors}
                                    {btn type="delete_selected" icon="icon-trash" dispatch="dispatch[affiliate_plans.m_delete]" form="affiliate_plan_form"}
                                ...
                                {include file="pickers/companies/picker.tpl"
                                    extra_var="affiliate_plans.add_companies?plan_id=$id&selected_section=linked_vendors"|escape:url
                                    data_id="affiliate"
                                    multiple=true
                    ...
                    <table class="table table-middle">
                        <thead>
                            <tr>
                                <th class="left plan-update-check-items">
                                    {include file="common/check_items.tpl" check_target="vendor"}</th>
                                <th class="plan-update-product-name">{__("vendor_name")}</th>
                                {if $payout_types}
                                    <th class="plan-update-product-commissions">{__("sales_commission")}</th>
                                    <th>&nbsp;</th>
                                {/if}
                        ...
                        {if $linked_vendors}
                            {foreach from=$linked_vendors key=company_id item=company}
                                <tr>
                                    <td class="left"><input type="checkbox" name="affiliate_plan[company_ids][]" value="{$company_id}" class="cm-item-company" />
                                    </td>
                                    <td>
                                        {if !"ULTIMATE"|fn_allowed_for}
                                            <a href="{"companies.update?company_id=`$company_id`"|fn_url}">{$company.company nofilter}</a>
                                        {/if}
                                        ...
                                        {if "ULTIMATE"|fn_allowed_for}
                                            {if $allow_save}
                                                <a href="{"companies.update?company_id=`$company_id`"|fn_url}">{$company.company nofilter}</a>
                                            {else}
                                                {$company.company nofilter}
                                            {/if}
                                        {/if}
                                    </td>
                                    {if $payout_types}
                                        <td class="nowrap">
                                            <input type="text" name="affiliate_plan[company_sales][{$company_id}][value]" value="{$company.sale.value}" size="10" class="input-small" />&nbsp;
                                            <select name="affiliate_plan[company_sales][{$company_id}][value_type]" class="span2">
                                                {foreach from=$payout_types.sale.value_types key="value_type" item="name_lang_var"}
                                                    <option value="{$value_type}" {if $company.sale.value_type==$value_type}selected="selected"{/if}>{__($name_lang_var)} {if $value_type == "A"}({$currencies.$primary_currency.symbol nofilter}){elseif $value_type == "P"}(%){/if}</option>
                                                {/foreach}
                                            </select>
                                        </td>
                                        <td class="nowrap right plan-update-tools">
                                            {capture name="tools_list"}
                                                ...
                                                {if !"ULTIMATE"|fn_allowed_for}
                                                    <li>{btn type="list" class="cm-confirm" text=__("delete") href="affiliate_plans.delete?company_id=`$company_id`&plan_id=`$id`"}</li>
                                                {/if}
                ...
                <!--content_linked_vendors--></div>
                {** /Linked vendors **}
            {/if}

            {if $addons.sd_affiliate.use_multiple_promotions == 'Y'}
                {** Coupons **}
                            ...
                            <table class="table table-middle">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="plan-update-coupon-name-picker">{__("name")}</th>
                                        <th class="plan-update-use-coupon-commissions-picker">{__("use_coupons_commission")}</th>
                                        <th class="plan-update-valid-coupon-picker">{__("valid")}</th>
                                    </tr>
                                </thead>
                                {if $coupons}
                                    ...
                                    <tr>
                                        ...
                                        {if $coupon.from_date == 0 && $coupon.to_date == 0}
                    ...
                    <table class="table table-middle">
                        <thead>
                            <tr>
                                ...
                                <th class="plan-update-coupon-name">{__("coupon")}</th>
                                <th>{__("use_coupons_commission")}</th>
                                <th class="plan-update-valid-coupon">{__("valid")}</th>
                                <th class="plan-update-tools">&nbsp;</th>
                            </tr>
                        </thead>
                        {if $affiliate_plan.coupons}
                            {foreach from=$affiliate_plan.coupons item=coupon}
                                <tr>
                                    ...
                                    <td class="plan-update-coupon-name-td">
                                        {if $allow_save}
                ...
                {** /Coupons **}
            {/if}
            {hook name="affiliate_plans:tabs_content"}{/hook}
        {/if}
    </form>
                        ...
                        {if $payout_data.default == "Y"}
                            ...
                            <div class="control-group">
                                ...
                                <div class="controls">
                                    ...
                                    <select name="affiliate_plan[payout_types][{$payout_data.id}][value_type]">
                                        {foreach from=$payout_data.value_types key="value_type" item="name_lang_var"}
                                            <option value="{$value_type}" {if $affiliate_plan.payout_types.$payout_id.value_type==$value_type}selected="selected"{/if}>{__($name_lang_var)} {if $value_type == "P"}(%){/if}</option>
                                        {/foreach}
                                    </select>
                                </div>
                            </div>
