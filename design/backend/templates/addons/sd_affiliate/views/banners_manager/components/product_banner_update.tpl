...
{ldelim}
    ...
    $.ceAjax('request', '{"aff_banner.view"|fn_url nofilter}' +
        ...
        '&to_price=' + ($('#elm_aff_product_banner_to_price').prop('checked') ? 'Y' : 'N')+
        '&to_cart=' + $('#elm_aff_product_banner_to_cart').val()+
        '&type=js', {ldelim}result_ids: 'id_example_banner', force_exec: true{rdelim}
    );
{rdelim}
//]]>
</script>
...
{capture name="preview_banner_content"}
    <div id="id_example_banner">
        <div id="affiliate_banner_container_{$banner.banner_id}"></div>
        {if $banner.code}{$banner.code nofilter}{/if}
    <!--id_example_banner--></div>
    <input id="id_banner_type" type="hidden" value="js" />
<p><a onclick="fn_get_example_banner(); return false;">{__("refresh_banner")}</a></p>
{/capture}
{capture name="adv_buttons"}
    <a id="opener_preview_banner" data-ca-target-id="content_preview_banner" class="cm-dialog-opener btn" title="{__("preview")}">
        <i class="exicon-preview"></i>
    </a>
    <div class="hidden" id="content_preview_banner" data-ca-keep-in-place="true">
        {$smarty.capture.preview_banner_content nofilter}
    <!--content_preview_banner--></div>
{/capture}
...
<div class="control-group">
    <label class="control-label" for="elm_aff_product_banner_to_price">{__("price")}:</label>
    <div class="controls">
    <label class="checkbox">
        <input type="hidden" name="banner[to_price]" value="N" />
        <input type="checkbox" id="elm_aff_product_banner_to_price" name="banner[to_price]" {if $banner.to_price == "Y"}checked="checked"{/if} value="Y" />
    </label>
    </div>
</div>
