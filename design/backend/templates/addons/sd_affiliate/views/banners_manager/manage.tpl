...
<div class="table-responsive-wrapper" id="content_{$link_to}">
            ...
            <table width="100%" class="table table-middle table-responsive">
                ...
                {foreach from=$banners item=c_banner}
                    <tr class="cm-row-status-{$c_banner.status|lower}">
                        ...
                        <td class="nowrap" data-th="{__("title")}">
                        ...
                        {if $banner_type == "T"}
                            <td class="center" data-th="{__("show_title")}">
                                <input type="hidden" name="banners_data[{$c_banner.banner_id}][show_title]" value="N" />
                                <input type="checkbox" name="banners_data[{$c_banner.banner_id}][show_title]" {if $c_banner.show_title == "Y"}checked="checked"{/if} value="Y" />
                            </td>
                        {/if}
                        {if $banner_type == "T" || $banner_type == "P"}
                           <td data-th="{__("width")}"><input type="text" name="banners_data[{$c_banner.banner_id}][width]" value="{$c_banner.width}" size="10" class="input-mini input-hidden" /></td>
                           <td data-th="{__("height")}"><input type="text" name="banners_data[{$c_banner.banner_id}][height]" value="{$c_banner.height}" size="10" class="input-mini input-hidden" /></td>
                        {/if}

                        {if $banner_type != "P"}
                            <td data-th="{__("product_groups")}">
                                {if $link_to == "C"}
                                    ...
                                    {/foreach}
                                {elseif $link_to == "P"}
                                    {foreach from=$c_banner.products key="item_id" item="item_name" name="fe"}
                                        <a href="{"products.update?product_id=`$item_id`"|fn_url}" class="row-status">{$item_name}</a>{if !$smarty.foreach.fe.last}, {/if}
                                    {/foreach}
                                {elseif $link_to == "G"}
                        ...
                        {/if}
                        <td class="center" data-th="{__("new_window")}">
                        ...
                        {if $banner_type == "P"}
                            <td class="center" data-th="{__("add_to_cart")}">
                                <input type="hidden" name="banners_data[{$c_banner.banner_id}][to_cart]" value="N" />
                                <input type="checkbox" name="banners_data[{$c_banner.banner_id}][to_cart]" {if $c_banner.to_cart == "Y"}checked="checked"{/if} value="Y"/>
                            </td>
                        {/if}
...
{include file="common/mainbox.tpl" title="{__("affiliate_banners")}: `$title`" title_start="{__("affiliate_banners")}"
   title_end=$title  content=$smarty.capture.mainbox adv_buttons=$smarty.capture.adv_buttons buttons=$smarty.capture.buttons}
