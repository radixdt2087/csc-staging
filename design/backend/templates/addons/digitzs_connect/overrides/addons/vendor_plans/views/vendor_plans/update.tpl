...
{capture name="tabsbox"}
    ...
    <div id="content_plan_commission_{$id}">
        ...
        <div class="control-group">
            <label class="control-label" for="elm_commission_{$id}"> Min Transaction Fee:*</label>
            <div class="controls">
                <input id="elm_commission_{$id}" type="text" name="plan_data[commission]" class="input-mini" value="{floatval($plan.commission)}" size="4"> % + {$currencies.$primary_currency.symbol nofilter} <input type="text" name="plan_data[fixed_commission]" value="{$plan.fixed_commission}" class="input-mini" size="4"> </br>
                <p>*Transactions fees increased on a per product basis this is only the min fee you will be charged. </p>
            </div>
