...
{if $addons.ab__hide_product_description.hide_in_product == "YesNo::YES"|enum}
{strip}
...
<input type="hidden" name="tab_data[ab__smc_hide_content][{$device}]" value="N">
<input type="checkbox" class="btn-group-checkbox__checkbox"{if $tab_data.ab__smc_hide_content.{$device}|default:"Y" == "Y"} checked{/if} id="elm_ab__smc_{$html_id}_hide_on_{$device}" name="tab_data[ab__smc_hide_content][{$device}]" value="Y">
