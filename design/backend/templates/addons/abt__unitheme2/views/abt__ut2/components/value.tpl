...
<input{if $label} title="{$label}"{/if} id="{$f_id}" type="checkbox" value="Y" name="{$f_v}"{if $f_value == 'Y'} checked="checked"{/if} {$f_disabled}>
{** selectbox **}
{elseif $f.type == 'selectbox'}
<select{if $label} title="{$label}"{/if} id="{$f_id}" name="{$f_v}" class="{$f.class|default:'span10'}" {$f_disabled}>
...
{** input **}
{elseif $f.type == 'input'}
<input{if $label} title="{$label}"{/if} id="{$f_id}" type="text" name="{$f_v}" value="{$f_value}" class="cm-trim {$f.class|default:'span10'}" {$f_disabled}>
{if $f.suffix}&nbsp;{$f.suffix}{/if}
{** textarea **}
{elseif $f.type == 'textarea'}
<textarea{if $label} title="{$label}"{/if} id="{$f_id}" name="{$f_v}" class="cm-trim {$f.class|default:'span10'}" {$f_disabled}>{$f_value}</textarea>
{** colorpicker **}
