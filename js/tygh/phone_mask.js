(function (_, $) {
    ...
    if (is_international_format || is_custom_format) {
      loadPhoneMasks().then(function (phone_masks) {
        ...
        $phone_elems.each(function (index, elm) {
          if (is_custom_format && $(elm).data('enableCustomMask')) {
           ...
           if(elm.id == 'elm_9' || elm.id == 'elm_31' || elm.id == 'litecheckout_phone' || elm.id == 'litecheckout_b_phone'){
           
              $(elm).val(this.value.replace(/\D/g, '').slice(-10));
              $(elm).inputmask({ 
                mask: phones, 
                greedy: false,
                showMaskOnHover: false,
      ...
      $.ceFormValidator('registerValidator', {
        ...
        func: function func(id) {
          if(id == "elm_9"){
            return validateLength($('#' + id));
          }else if(id == "elm_31" || id == 'litecheckout_phone' || id == 'litecheckout_b_phone'){
            return validatePhoneForLength('#' + id);
          }else{
  ...
  function validateLength($input){
    if(($input.val().replace(/\D/g, '')).length < 10){
      return false;
    }else{
      return true;
    }
  ...
  function validatePhoneForLength($input){
    if(($($input).val().replace(/\D/g, '')).length == 0){
      return true;
    }else if(($($input).val().replace(/\D/g, '')).length > 1 && ($($input).val().replace(/\D/g, '')).length < 10){
      return false;
    }else{
      return true;
    }
