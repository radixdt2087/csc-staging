 ...
 */
function fn_ach_payments_get_payments($params, $fields, $join, $order, &$condition, $having)
{
    ...
    if ($mode == 'checkout') {
        ...
        $pdata = db_get_field(
            'SELECT ach_token'
            .' FROM ?:ach_details WHERE user_id = ?i AND isDefault = 1 AND isDeleted = 0', $dauth['user_id']
        );
        $token = generateTokenCardManagement();
        $token=$token->data->attributes->appToken;
        if(!empty($pdata)){
            $defaultCard = getACHDetailsManagement($pdata,$token);
            Tygh::$app['view']->assign('defaultCard',$defaultCard);
            $savedCards = db_get_array('SELECT * FROM ?:ach_details where user_id = ?i AND isDeleted = 0', $dauth['user_id']);
          ...
          //  echo '<pre>'; print_r($savedCards); die();
            Tygh::$app['view']->assign('ach_details',$cardDetails);
        } 
    }
}
function generateTokenACHManagement() {
    ...
    return callDigitzApiACHManagement($tokenApi,$post,$header);
}

function callDigitzApiACHManagement($api,$post,$header) {
    $curl = curl_init();
