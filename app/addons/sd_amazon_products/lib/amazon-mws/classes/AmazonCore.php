...
<ErrorResponse xmlns="http://mws.amazonaws.com/doc/2009-01-01/">
  ...
  <RequestID>123</RequestID>
    ...
    public function setConfig($path){
        if (file_exists($path) && is_readable($path)){
            ...
            if (isset($AMAZON_SERVICE_URL)) {
                ...
                $this->urlbasepub = rtrim($AMAZON_SERVICE_URL, '/') . '/';
            }
        } else {
     ...
     * 
     * This method sets a number of key values from the config. These values
     ...
     */
    public function setStore($_auth){
        if (is_array($_auth) && array_key_exists('merchantId', $_auth)){
            $this->options['SellerId'] = $_auth['merchantId'];
        } else {
            $this->log("Merchant ID is missing!",'Warning');
        }

        if (is_array($_auth) && array_key_exists('keyId', $_auth)){
            $this->options['AWSAccessKeyId'] = $_auth['keyId'];
        } else {
            $this->log("Access Key ID is missing!",'Warning');
        }

        if (is_array($_auth) && array_key_exists('secretKey', $_auth)){
            $this->secretKey = $_auth['secretKey'];
        } else {
            $this->log("Secret Key ID is missing!",'Warning');
        }

        if (is_array($_auth) && array_key_exists('marketplaceId', $_auth)){
            $this->options['MarketplaceId'] = $_auth['marketplaceId'];
        }

        if (!empty($_auth['serviceUrl'])) {
            $this->urlbase = $_auth['serviceUrl'];
        }
        
        if (!empty($_auth['MWSAuthToken'])) {
            $this->options['MWSAuthToken'] = $_auth['MWSAuthToken'];
        }
    }
    ...
    protected function sendRequest($url,$param){
        ...
        if ($response['code'] == '503' && $this->throttleStop==false){
            fn_set_notification('E', 'Throttled by Amazon', 'You exceeded your quota of requests.');
            //$this->sleep();
            exit;
        }
