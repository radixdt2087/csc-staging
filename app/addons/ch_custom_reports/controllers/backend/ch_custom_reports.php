...
if($mode ==  'vendor_reports') 
{
    ...
    if ($user_id) {
        ...
        CURLOPT_HTTPHEADER => array(
            ...
            "postman-token: b7522d0e-5664-87ec-78dc-638338f26f34"
        ),
        ));
        ...
        $view->assign(['reports' => $reports, 'myEarnings' => $myEarningsreports, 'getOverview' => $getOverview, 'orderLink' => $orderLink, 'lpoUserData' => $lporeports, 'hierarchyresponse' => $html, 'memberData' => $memberData]);
...
function fn_my_changes_get_cscart_users_details($user_id)
{
    ...
    if($user_id != "")
    {
        ...
        //{$lpoUserData|@fn_print_r}
        //echo $csreports['users'][0]['user_type'];
...
{
    preg_match( "/(-+)?\d+(\.\d{1,3})?/" ,$price ,$pricePoint);
    echo $pricePoint[0];
}
