...
if ($mode == 'update') {
    ...
    $userdata = fn_get_user_info($auth['user_id']);
   // print_r( $userdata['lastname']); die();
     if($row = mysqli_fetch_assoc($vendor_details)) {
        $row['user_email'] = $user_email;
        $user_data['firstname'] = $userdata['firstname'];
        $user_data['lastname'] = $userdata['lastname'];
        $row['content'] = $content;
        Tygh::$app['view']->assign(
            'user_data',
            $user_data
...
function getMerchantDetails($company_details) {
    ...
    $ip_addressDigitzs = explode(":",$ip_address);
...
function displayError($mer) {
    ...
    if($mer->source->pointer == '/data/attributes/businessInfo/ein') {
        $error = '"ein" must be 9 digits long and should be numeric from 0-9';
    } else if($mer->source->pointer == '/data/attributes/bankInfo/routingNumber') {
        $error = '"routingNumber" must be 9 digits long and should be numeric from 0-9';
    } else if($mer->source->pointer =='/data/attributes/personalInfo/socialSecurity') {
        $error = '"socialSecurity" must be 9 digits long and should be numeric from 0-9';
...
function sendMail($to,$company_id) {
    ...
    $success = $mailer->send(array(
            ...
            'tpl' => 'addons/digitzs_connect/addons_mail.tpl',
    ), 'A');
