...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
 // For deleting the Card
    ...
    if (defined('AJAX_REQUEST')) {
      ...
      if (!empty($card_token)) {
          ...
          if( $isDefault == 1 ){
            ...
            $result = db_query("UPDATE ?:card_details SET ?u WHERE card_token = ?s AND id = ?i", $deleteArray, $card_token, $card_id);
            if ($result == 1) {
              fn_set_notification('N', __('notice'), 'Your Card is deleted');
    ...
    if (defined('AJAX_REQUEST')) {
        ...
        if(isset($cardToken->errors)) {
          foreach($cardToken->errors as $mer) {
                ...
                $error_status=true;
                if (defined('AJAX_REQUEST')) {
                  Tygh::$app['ajax']->assign('error', $error);
                    exit;
                } else {
                    return [CONTROLLER_STATUS_REDIRECT, $force_redirection];
                }
  ...
  if ($mode == 'update') {
    if (defined('AJAX_REQUEST')) { 
      if($_REQUEST['number'] == ''){
        fn_set_notification('E', __('error'), 'Please enter the card number');
      }else{
        $card_id = db_get_field('SELECT id FROM ?:card_details where card_token = ?s', $_REQUEST['card_token']);
        $data = array(
          'card_num' => str_replace(' ','',$_REQUEST['number']),
          'card_type' => $_REQUEST['card_type'],
          'card_holder_name'=> $_REQUEST['holder'],
          'valid_thru' => $_REQUEST['month'].$_REQUEST['year'],
  ...
  if ($mode == 'setDefault') {
    if (defined('AJAX_REQUEST')) {
      ...
      if (!empty($card_token)){
          $isDefault = db_get_field('SELECT isDefault FROM ?:card_details where card_token = ?s', $card_token);
          if( $isDefault == 1 ){
            fn_set_notification('W', __('warning'), 'This card is already selected as default.');
          }
          else{
            $card_id = db_get_field('SELECT id FROM ?:card_details where card_token = ?s', $card_token); 
            $removeDefault = db_query("UPDATE ?:card_details SET isDefault = '0' WHERE user_id = ?i", $dauth['user_id']);
            if ($removeDefault  != 0) {
                $isDefault = db_query("UPDATE ?:card_details SET isDefault = '1' WHERE id = ?i AND user_id = ?i", $card_id,$dauth['user_id']);
                if ($isDefault == 1) {
                  $force_redirection = 'checkout.checkout';
                  if (defined('AJAX_REQUEST')) {
                    fn_set_notification('N', __('notice'), 'This card is set as default.');
...
function displayError($mer) {
  $error='';
  if($mer->source->pointer == '/data/attributes/card/number') {
      $error = 'Declined. The card number is incorrect. Verify the card number and re-enter.';
  } else if($mer->detail == 'data.attributes.card.number') {
      $error = 'Declined. The card number is incorrect. Verify the card number and re-enter.';
  } else {
      $error = $mer->detail;
  }
